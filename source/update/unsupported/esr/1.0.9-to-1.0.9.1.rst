...
\*\*recovery-fd.schema\*\*, \*\*asterisk-fd.schema\*\* and
\*\*asterisk-fd-conf.schema\*\* because asterisk plugin does not exist
anymore.
...
fusiondirectory-setup --check-deprecated will output a list of dn using
old attributes and objectClasses

``fusiondirectory-setup`` ``--check-deprecated`` ``List`` ``LDAP``
``entries`` ``using`` ``deprecated`` ``attributes`` ``or``
``objectclasses``
``cn=fusiondirectory,ou=configs,dc=fusiondirectory,dc=org`` ``contains``
``an`` ``obsolete`` ``attribute``
``uid=fd-admin,ou=people,dc=fusiondirectory,dc=org`` ``uses`` ``the``
...
-  
   ...
   -  fusiondirectory-setup --migrate-users\*\* can help you migrate you
      ...
      automatically migrate them and you just need to install the
      \*\*personal\*\* plugin
