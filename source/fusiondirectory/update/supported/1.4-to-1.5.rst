...
Upgrade FusionDirectory Tools First
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Upgrade the fusiondirectory-tools to get the latest version of the fusiondirectory-tools

.. code-block:: shell

   apt-get install fusiondirectory-integrator
   apt-get install fusiondirectory-tools
